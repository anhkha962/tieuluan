...
class AdminModel extends DB
{
    ...
    public function UpdateSanpham($id,$tensp,$giasp,$tendm,$tenth,$tonkho,$noibat,$mota,$ha1){
        $path = "/tieuluanmvc/mvc/public/product/";
        if(isset($_GET['id'])){
            $sqlDel = "UPDATE `sanpham` SET `sp_ten`='$tensp',`nhom_id`='$tendm',`sp_gia`='$giasp',`sp_hinh`='$path$ha1',`sp_mota`='$mota',`sp_trangthai`='$tonkho',`noibat`='$noibat',`thuonghieu`='$tenth' WHERE ".$_GET["$id"]."";
            mysqli_query($this->con, $sqlDel);
            header('Location: ./XemSanpham');   
        }
    }
    public function AddNhanvien($name,$username,$date,$email,$tel,$pass){
        ...
        if(isset($_POST['themnv'])){
            ...
            if(mysqli_query($this->con,"INSERT INTO `taikhoan`(`taikhoan`, `matkhau`, `quyen`) VALUES ('$username','$pass','2')")){
               mysqli_query($this->con,"INSERT INTO `nhanvien`(`nv_ten`, `nv_email`, `nv_ngaysinh`, `nv_phone`, `tk` ) VALUES ('$name','$email','$date','$tel','$username')");
               echo'<script language="javascript">alert("Thêm nhân viên thành công!"); window.location="./XemTaikhoanNV";</script>';
            }else{
                '<script language="javascript">alert("Thêm nhân viên không thành công!"); window.history.back(-1);</script>';
            }
        }
    }
    ...
    $query = mysqli_query($this->con,"SELECT taikhoan, matkhau FROM taikhoan WHERE taikhoan= '$username' AND quyen = '1'");
